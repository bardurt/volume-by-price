...
def start(symbol, asset, method):
    ...
    if asset == "c":
        ...
        api_key = read_api_key("../config/avkey.txt")
        if not api_key:
...
def show_help():
    ...
    print("\nNote:")
    print("If using Alpha Vantage (stocks), make sure you have your API key saved in a file named 'avkey.txt' in the the config folder [root/config/avkey.txt].")
    print("-----------------------------------------------------")
    print("")
